...
Public Function IVar(Value As Variant, Tipo As EDataType) As String
  IVar = "NULL"
  If IsNull(Value) Then Exit Function
  Select Case Tipo
    Case DtNumDecimal, DtNumEntero, DtMoneda: IVar = DB.INum(Value)
    Case DtGeneral, DtPassword, DtMayusculas: IVar = DB.IStr(Value)
    Case DtFecha, DtHora, DtFechaHora: IVar = DB.IFec(Value)
    Case DtNumTexto: If Value <> "" Then IVar = DB.INum(Val(Value))
    Case DtStrTexto: IVar = DB.IStr(GetCodigo(CStr(Value)))
    Case Else: IVar = CStr(Value)
  End Select
End Function

Public Function ReplaceNULLS(ByVal SQL As String) As String
  Dim J As Integer
  Dim S As String
  J = InStr(SQL, "WHERE")
  If J > 0 Then
    S = Mid$(SQL, J + 5)
    S = Replace(S, "= NULL", " IS NULL")
    S = Replace(S, "=NULL", " IS NULL")
    SQL = Left$(SQL, J + 4) & S
  End If
  ReplaceNULLS = SQL
